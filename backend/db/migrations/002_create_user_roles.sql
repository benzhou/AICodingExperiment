...
BEGIN
    IF (TG_OP = 'DELETE') THEN
        ...
        SELECT OLD.id, OLD.user_id, OLD.role, 'DELETE', (NOW() AT TIME ZONE 'UTC'), NULL;
        RETURN OLD;
    ELSIF (TG_OP = 'UPDATE') THEN
        INSERT INTO user_roles_audit
        SELECT NEW.id, NEW.user_id, NEW.role, 'UPDATE', (NOW() AT TIME ZONE 'UTC'), NULL;
        RETURN NEW;
    ELSIF (TG_OP = 'INSERT') THEN
        INSERT INTO user_roles_audit
        SELECT NEW.id, NEW.user_id, NEW.role, 'INSERT', (NOW() AT TIME ZONE 'UTC'), NULL;
        RETURN NEW;
    END IF;
    RETURN NULL;
END;
