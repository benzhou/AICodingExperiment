...
BEGIN
    IF (TG_OP = 'DELETE') THEN
        ...
        SELECT OLD.id, OLD.data_source_id, OLD.transaction_date, OLD.amount, OLD.status, OLD.match_id, 'DELETE', NOW(), NULL;
        RETURN OLD;
    ELSIF (TG_OP = 'UPDATE') THEN
        INSERT INTO transactions_audit
        SELECT NEW.id, NEW.data_source_id, NEW.transaction_date, NEW.amount, NEW.status, NEW.match_id, 'UPDATE', NOW(), NULL;
        RETURN NEW;
    ELSIF (TG_OP = 'INSERT') THEN
        INSERT INTO transactions_audit
        SELECT NEW.id, NEW.data_source_id, NEW.transaction_date, NEW.amount, NEW.status, NEW.match_id, 'INSERT', NOW(), NULL;
        RETURN NEW;
    END IF;
    RETURN NULL;
END;
