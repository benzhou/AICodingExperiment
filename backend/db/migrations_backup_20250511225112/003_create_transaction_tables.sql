...
CREATE TABLE IF NOT EXISTS transaction_uploads (
    ...
    upload_date TIMESTAMP NOT NULL DEFAULT set_utc_timestamp(),
...
BEGIN
    IF (TG_OP = 'DELETE') THEN
        ...
        SELECT OLD.id, OLD.data_source_id, OLD.transaction_date, OLD.amount, OLD.status, OLD.match_id, 'DELETE', set_utc_timestamp(), NULL;
        RETURN OLD;
    ELSIF (TG_OP = 'UPDATE') THEN
        INSERT INTO transactions_audit
        SELECT NEW.id, NEW.data_source_id, NEW.transaction_date, NEW.amount, NEW.status, NEW.match_id, 'UPDATE', set_utc_timestamp(), NULL;
        RETURN NEW;
    ELSIF (TG_OP = 'INSERT') THEN
        INSERT INTO transactions_audit
        SELECT NEW.id, NEW.data_source_id, NEW.transaction_date, NEW.amount, NEW.status, NEW.match_id, 'INSERT', set_utc_timestamp(), NULL;
        RETURN NEW;
    END IF;
    RETURN NULL;
END;
